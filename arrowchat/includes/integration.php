<?php
	...
	*/
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/files/' . $image)) 
		{
			return '/homesimex/files/' . $image;
		} 
		else 
		{
	...
	{	
		...
		$result = $db->execute("
			...
			WHERE id_users = '" . $db->escape_string($userid) . "'
		");
	...
	{	
		...
		{	 
			while ($row = $db->fetch_array($result))
			{
				...
				$groups[] = array($row['id_grupo'], $row['nombre_grupo']);
			}
