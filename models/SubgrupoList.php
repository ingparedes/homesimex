...
class SubgrupoList extends Subgrupo
{
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "SubgrupoAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->descripcion_subgrupo); // descripcion_subgrupo
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'users')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("UsersList?" . Config("TABLE_SHOW_MASTER") . "=subgrupo&" . GetForeignKeyUrl("fk_id_subgrupo", $this->id_subgrupo->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("UsersGrid");
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'subgrupo')) {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                ...
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'subgrupo') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
    ...
    protected function renderListOptionsExt()
    {
        ...
        $links = "";
        $btngrps = "";
        $sqlwrk = "`subgrupo`=" . AdjustSql($this->id_subgrupo->CurrentValue, $this->Dbid) . "";

        // Column "detail_users"
        if ($this->DetailPages && $this->DetailPages["users"] && $this->DetailPages["users"]->Visible) {
            $link = "";
            $option = $this->ListOptions["detail_users"];
            $url = "UsersPreview?t=subgrupo&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"users\" data-url=\"" . $url . "\" class=\"btn-group btn-group-sm\">";
            if ($Security->allowList(CurrentProjectID() . 'subgrupo')) {
                ...
                $links .= $link;
                $detaillnk = JsEncodeAttribute("UsersList?" . Config("TABLE_SHOW_MASTER") . "=subgrupo&" . GetForeignKeyUrl("fk_id_subgrupo", $this->id_subgrupo->CurrentValue) . "");
                $btngrp .= "<button type=\"button\" class=\"btn btn-default\" title=\"" . $Language->TablePhrase("users", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "';return false;\">" . $Language->phrase("MasterDetailListLink") . "</button>";
            }
            $detailPageObj = Container("UsersGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'subgrupo')) {
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'subgrupo')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=users");
                $btngrp .= "<button type=\"button\" class=\"btn btn-default\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</button>";
            }
