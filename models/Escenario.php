...
class Escenario extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // evento_asociado
        $this->evento_asociado = new DbField('escenario', 'escenario', 'x_evento_asociado', 'evento_asociado', '`evento_asociado`', '`evento_asociado`', 200, 60, -1, false, '`evento_asociado`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        $this->evento_asociado->Sortable = true; // Allow sort
        $this->evento_asociado->SelectMultiple = true; // Multiple select
        switch ($CurrentLanguage) {
            case "en":
                $this->evento_asociado->Lookup = new Lookup('evento_asociado', 'view_evento_asociado', false, 'id_incidente', ["inc_es","","",""], [], [], [], [], [], [], '', '');
                break;
            case "es":
                $this->evento_asociado->Lookup = new Lookup('evento_asociado', 'view_evento_asociado', false, 'id_incidente', ["inc_es","","",""], [], [], [], [], [], [], '', '');
                break;
        ...
        $this->evento_asociado->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->evento_asociado->Param, "CustomMsg");
        $this->Fields['evento_asociado'] = &$this->evento_asociado;
        ...
        $this->fechaini_real->Required = true; // Required field
        ...
        $this->fechafinal_real->Required = true; // Required field
        ...
        $this->fechaini_simulado->Required = true; // Required field
        ...
        $this->fechafin_simulado->Required = true; // Required field
        ...
        $this->estado->AdvancedSearch->SearchValueDefault = 1;
        $this->estado->AdvancedSearch->SearchOperatorDefault = "LIKE";
        $this->estado->AdvancedSearch->SearchOperatorDefault2 = "";
        $this->estado->AdvancedSearch->SearchConditionDefault = "AND";
        ...
        // entrar
        $this->entrar = new DbField('escenario', 'escenario', 'x_entrar', 'entrar', '\'id_escenario\'', '\'id_escenario\'', 200, 12, -1, false, '\'id_escenario\'', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function getSqlWhere() // Where
    {
        ...
        $this->DefaultFilter = (CurrentUserInfo("perfil") > 1 ) ? "id_escenario = '".CurrentUserInfo("escenario")."'" : "";
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['id_escenario']) && $rsold['id_escenario'] != $rs['id_escenario'])) { // Update detail field 'escenario'
            $cascadeUpdate = true;
            $rscascade['escenario'] = $rs['id_escenario'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("users")->loadRs("`escenario` = " . QuotedValue($rsold['id_escenario'], DATATYPE_NUMBER, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'id_users';
                $rskey[$fldname] = $rsdtlold[$fldname];
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        $dtlrows = Container("users")->loadRs("`escenario` = " . QuotedValue($rs['id_escenario'], DATATYPE_NUMBER, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->evento_asociado->DbValue = $row['evento_asociado'];
...
SORTHTML;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->evento_asociado);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->evento_asociado);
    ...
    {
        // Example:
       // $this->estado->AdvancedSearch->SearchValue = "1"; // Search value
    }
