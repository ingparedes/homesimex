...
class UsersPreview extends Users
{
    ...
    // Page object name
    public $PageObjName = "UsersPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "subgrupo") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=subgrupo&" . GetForeignKeyUrl("fk_id_subgrupo", $this->subgrupo->QueryStringValue) . "";
        }
        if ($masterTblVar == "grupo") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=grupo&" . GetForeignKeyUrl("fk_id_grupo", $this->grupo->QueryStringValue) . "";
        }
        if ($masterTblVar == "escenario") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=escenario&" . GetForeignKeyUrl("fk_id_escenario", $this->escenario->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeysFromFilter($f)
    {
        ...
        if ($masterTblVar == "subgrupo") {
            ...
            if ($x !== false) {
                ...
                $val = $this->unquoteValue($val, "DB");
                 $this->subgrupo->setQueryStringValue($val);
            }
        }
        if ($masterTblVar == "grupo") {
            $find = "`grupo`=";
        ...
        if ($masterTblVar == "escenario") {
            ...
            if ($x !== false) {
                ...
                $val = substr($f, $x);
                $val = $this->unquoteValue($val, "DB");
                 $this->escenario->setQueryStringValue($val);
            }
        }
    }
