...
class TareasAdd extends Tareas
{
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->valoracion->CurrentValue = null;
        $this->valoracion->OldValue = $this->valoracion->CurrentValue;
        $this->color->CurrentValue = "blue";
    }
    ...
    protected function newRow()
    {
        ...
        $row['valoracion'] = $this->valoracion->CurrentValue;
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("MensajesGrid");
            if (in_array("mensajes", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->id_tarea->setSessionValue($this->id_tarea->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "mensajes"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->id_tarea->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    public function pageRedirecting(&$url)
    {
        // Example:
        //$url = "your URL";
        if (@$_POST["xxx"] <> "") // Check if the page is doing insert and check if the 2nd button is pressed
    $url = "TareasAdd?showdetail";
