 ...
 * Page class
 */
class UserEmailEdit extends UserEmail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UserEmailEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (user_email)
        if (!isset($GLOBALS["user_email"]) || get_class($GLOBALS["user_email"]) == PROJECT_NAMESPACE . "user_email") {
            $GLOBALS["user_email"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'user_email');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("user_email"));
                $doc->Text = @$content;
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->id_user_email->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_user_email") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_user_email->setQueryStringValue($keyValue);
                $this->id_user_email->setOldValue($this->id_user_email->QueryStringValue);
            } elseif (Post("id_user_email") !== null) {
                $this->id_user_email->setFormValue(Post("id_user_email"));
                $this->id_user_email->setOldValue($this->id_user_email->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("id_user_email") ?? Route("id_user_email")) !== null) {
                    $this->id_user_email->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->id_user_email->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("UserEmailList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "UserEmailList") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("UserEmailList"), "", $this->TableVar, true);
        $pageId = "edit";
