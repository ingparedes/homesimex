...
class EscenarioAdd extends Escenario
{
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->evento_asociado->CurrentValue = null;
        $this->evento_asociado->OldValue = $this->evento_asociado->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'evento_asociado' first before field var 'x_evento_asociado'
        $val = $CurrentForm->hasValue("evento_asociado") ? $CurrentForm->getValue("evento_asociado") : $CurrentForm->getValue("x_evento_asociado");
        if (!$this->evento_asociado->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->evento_asociado->Visible = false; // Disable update for API request
            } else {
                $this->evento_asociado->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->evento_asociado->CurrentValue = $this->evento_asociado->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['evento_asociado'] = $this->evento_asociado->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->evento_asociado->ViewValue = $this->evento_asociado->Lookup !== null && is_array($this->evento_asociado->Lookup->Options) ? $curVal : null;
            }
            if ($this->evento_asociado->ViewValue !== null) { // Load from cache
                $this->evento_asociado->EditValue = array_values($this->evento_asociado->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                ...
                $sqlWrk = $this->evento_asociado->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->evento_asociado->EditValue = $arwrk;
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->evento_asociado->Required) {
            if ($this->evento_asociado->FormValue == "") {
                $this->evento_asociado->addErrorMessage(str_replace("%s", $this->evento_asociado->caption(), $this->evento_asociado->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // evento_asociado
        $this->evento_asociado->setDbValueDef($rsnew, $this->evento_asociado->CurrentValue, null, false);
        ...
        // entrar
        $this->entrar->setDbValueDef($rsnew, $this->entrar->CurrentValue, "", false);
        if ($this->image_escenario->Visible && !$this->image_escenario->Upload->KeepFile) {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("UsersGrid");
            if (in_array("users", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->escenario->setSessionValue($this->id_escenario->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "users"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->escenario->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("users", $detailTblVar)) {
                ...
                if ($detailPageObj->DetailAdd) {
                    ...
                    $detailPageObj->setCurrentMasterTable($this->TableVar);
                    $detailPageObj->setStartRecordNumber(1);
                    $detailPageObj->escenario->IsDetailKey = true;
                    $detailPageObj->escenario->CurrentValue = $this->id_escenario->CurrentValue;
                    $detailPageObj->escenario->setSessionValue($detailPageObj->escenario->CurrentValue);
