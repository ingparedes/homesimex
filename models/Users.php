...
class Users extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // escenario
        $this->escenario = new DbField('users', 'users', 'x_escenario', 'escenario', '`escenario`', '`escenario`', 3, 11, -1, false, '`escenario`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        $this->escenario->IsForeignKey = true; // Foreign key field
        $this->escenario->Sortable = true; // Allow sort
        $this->escenario->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->escenario->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        switch ($CurrentLanguage) {
            case "en":
                $this->escenario->Lookup = new Lookup('escenario', 'escenario', false, 'id_escenario', ["nombre_escenario","","",""], [], ["x_grupo"], [], [], [], [], '', '');
                break;
            case "es":
                $this->escenario->Lookup = new Lookup('escenario', 'escenario', false, 'id_escenario', ["nombre_escenario","","",""], [], ["x_grupo"], [], [], [], [], '', '');
                break;
        ...
        $this->escenario->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->escenario->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->escenario->Param, "CustomMsg");
        $this->Fields['escenario'] = &$this->escenario;
        ...
        switch ($CurrentLanguage) {
            case "en":
                $this->grupo->Lookup = new Lookup('grupo', 'grupo', false, 'id_grupo', ["nombre_grupo","","",""], ["x_escenario"], ["x_subgrupo"], ["id_escenario"], ["x_id_escenario"], [], [], '', '');
                break;
            case "es":
                $this->grupo->Lookup = new Lookup('grupo', 'grupo', false, 'id_grupo', ["nombre_grupo","","",""], ["x_escenario"], ["x_subgrupo"], ["id_escenario"], ["x_id_escenario"], [], [], '', '');
                break;
        ...
        $this->subgrupo->IsForeignKey = true; // Foreign key field
        ...
        $this->perfil->Required = true; // Required field
        ...
        // pw
        $this->pw = new DbField('users', 'users', 'x_pw', 'pw', '`pw`', '`pw`', 200, 200, -1, false, '`pw`', false, false, false, 'FORMATTED TEXT', 'PASSWORD');
    ...
    public function getMasterFilter()
    {
        // Master filter
        $masterFilter = "";
        if ($this->getCurrentMasterTable() == "subgrupo") {
            if ($this->subgrupo->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("`id_subgrupo`", $this->subgrupo->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
        if ($this->getCurrentMasterTable() == "grupo") {
            if ($this->grupo->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("`id_grupo`", $this->grupo->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
        if ($this->getCurrentMasterTable() == "escenario") {
            if ($this->escenario->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("`id_escenario`", $this->escenario->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilter()
    {
        ...
        if ($this->getCurrentMasterTable() == "subgrupo") {
            if ($this->subgrupo->getSessionValue() != "") {
                $detailFilter .= "" . GetForeignKeySql("`subgrupo`", $this->subgrupo->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
        ...
        if ($this->getCurrentMasterTable() == "escenario") {
            if ($this->escenario->getSessionValue() != "") {
                $detailFilter .= "" . GetForeignKeySql("`escenario`", $this->escenario->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    // Master filter
    public function sqlMasterFilter_subgrupo()
    {
        return "`id_subgrupo`=@id_subgrupo@";
    }
    // Detail filter
    public function sqlDetailFilter_subgrupo()
    {
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->escenario->DbValue = $row['escenario'];
    ...
    {
        if ($this->getCurrentMasterTable() == "subgrupo" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id_subgrupo", $this->subgrupo->CurrentValue ?? $this->subgrupo->getSessionValue());
        }
        if ($this->getCurrentMasterTable() == "grupo" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id_grupo", $this->grupo->CurrentValue ?? $this->grupo->getSessionValue());
        }
        if ($this->getCurrentMasterTable() == "escenario" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id_escenario", $this->escenario->CurrentValue ?? $this->escenario->getSessionValue());
        }
        return $url;
    }
...
SORTHTML;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->escenario);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->escenario);
    ...
    public function recordsetSelecting(&$filter)
    {
        ...
        $UserGroupo = CurrentUserInfo("grupo");
        $UserEscenario = CurrentUserInfo("escenario");
            if(CurrentUserLevel()== 3 || CurrentUserLevel()== 4){
              AddFilter($filter, "`id_users` ='". CurrentUserID()."' AND escenario = '".$UserEscenario."'");
              }
           elseif (CurrentUserLevel() == 2){
              AddFilter($filter, "`grupo` = '".$UserGroupo."' AND escenario = '".$UserEscenario."'");
              }
      }
