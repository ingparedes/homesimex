 ...
 * Page class
 */
class TareasView extends Tareas
{
    use MessagesTrait;
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_tarea") ?? Route("id_tarea")) !== null) {
                ...
                $this->RecKey["id_tarea"] = $this->id_tarea->QueryStringValue;
            } elseif (Post("id_tarea") !== null) {
                $this->id_tarea->setFormValue(Post("id_tarea"));
                $this->RecKey["id_tarea"] = $this->id_tarea->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_tarea->setQueryStringValue($keyValue);
                $this->RecKey["id_tarea"] = $this->id_tarea->QueryStringValue;
            } else {
                $returnUrl = "TareasList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "TareasList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "TareasList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        // "detail_mensajes"
        $item = &$option->add("detail_mensajes");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("mensajes", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("MensajesList?" . Config("TABLE_SHOW_MASTER") . "=tareas&" . GetForeignKeyUrl("fk_id_tarea", $this->id_tarea->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'tareas')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=mensajes"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'tareas')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=mensajes"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailEditLink")) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
        ...
        $item->Body = $body;
        $item->Visible = $Security->allowList(CurrentProjectID() . 'mensajes');
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("mensajes", $detailTblVar)) {
                ...
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->id_tarea->IsDetailKey = true;
                    $detailPageObj->id_tarea->CurrentValue = $this->id_tarea->CurrentValue;
                    $detailPageObj->id_tarea->setSessionValue($detailPageObj->id_tarea->CurrentValue);
                }
            }
        }
    }
