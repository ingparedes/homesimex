...
/**
 * Table class for send_email
 */
class SendEmail2 extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'send_email2';
        $this->TableName = 'send_email';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`send_email`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_sendemail
        $this->id_sendemail = new DbField('send_email2', 'send_email', 'x_id_sendemail', 'id_sendemail', '`id_sendemail`', '`id_sendemail`', 3, 11, -1, false, '`id_sendemail`', false, false, false, 'FORMATTED TEXT', 'NO');
        $this->id_sendemail->IsAutoIncrement = true; // Autoincrement field
        $this->id_sendemail->IsPrimaryKey = true; // Primary key field
        $this->id_sendemail->Sortable = true; // Allow sort
        $this->id_sendemail->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->id_sendemail->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->id_sendemail->Param, "CustomMsg");
        $this->Fields['id_sendemail'] = &$this->id_sendemail;

        // sujeto
        $this->sujeto = new DbField('send_email2', 'send_email', 'x_sujeto', 'sujeto', '`sujeto`', '`sujeto`', 200, 120, -1, false, '`sujeto`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // mensaje
        $this->mensaje = new DbField('send_email2', 'send_email', 'x_mensaje', 'mensaje', '`mensaje`', '`mensaje`', 201, 65535, -1, false, '`mensaje`', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
        ...
        // tiempo
        $this->tiempo = new DbField('send_email2', 'send_email', 'x_tiempo', 'tiempo', '`tiempo`', CastDateFieldForLike("`tiempo`", 15, "DB"), 135, 19, 15, false, '`tiempo`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->tiempo->Sortable = true; // Allow sort
        $this->tiempo->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_SEPARATOR"], $Language->phrase("IncorrectShortDateYMD"));
        $this->tiempo->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->tiempo->Param, "CustomMsg");
        $this->Fields['tiempo'] = &$this->tiempo;

        // archivo
        $this->archivo = new DbField('send_email2', 'send_email', 'x_archivo', 'archivo', '`archivo`', '`archivo`', 200, 100, -1, true, '`archivo`', false, false, false, 'FORMATTED TEXT', 'FILE');
        ...
        // status
        $this->status = new DbField('send_email2', 'send_email', 'x_status', 'status', '`status`', '`status`', 16, 1, -1, false, '`status`', false, false, false, 'FORMATTED TEXT', 'CHECKBOX');
        $this->status->Sortable = true; // Allow sort
        $this->status->DataType = DATATYPE_BOOLEAN;
        switch ($CurrentLanguage) {
            case "en":
                $this->status->Lookup = new Lookup('status', 'send_email2', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
            case "es":
                $this->status->Lookup = new Lookup('status', 'send_email2', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
        ...
        // de_user
        $this->de_user = new DbField('send_email2', 'send_email', 'x_de_user', 'de_user', '`de_user`', '`de_user`', 3, 11, -1, false, '`de_user`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->de_user->Sortable = true; // Allow sort
        $this->de_user->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->de_user->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->de_user->Param, "CustomMsg");
        $this->Fields['de_user'] = &$this->de_user;

        // copy_user
        $this->copy_user = new DbField('send_email2', 'send_email', 'x_copy_user', 'copy_user', '`copy_user`', '`copy_user`', 200, 30, -1, false, '`copy_user`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->copy_user->Sortable = true; // Allow sort
        $this->copy_user->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->copy_user->Param, "CustomMsg");
        $this->Fields['copy_user'] = &$this->copy_user;

        // para_user
        $this->para_user = new DbField('send_email2', 'send_email', 'x_para_user', 'para_user', '`para_user`', '`para_user`', 200, 60, -1, false, '`para_user`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        $this->para_user->Sortable = true; // Allow sort
        $this->para_user->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->para_user->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        switch ($CurrentLanguage) {
            case "en":
                $this->para_user->Lookup = new Lookup('para_user', 'users', false, 'id_users', ["nombres","apellidos","",""], [], [], [], [], [], [], '', '');
                break;
            case "es":
                $this->para_user->Lookup = new Lookup('para_user', 'users', false, 'id_users', ["nombres","apellidos","",""], [], [], [], [], [], [], '', '');
                break;
        ...
        $this->para_user->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->para_user->Param, "CustomMsg");
        $this->Fields['para_user'] = &$this->para_user;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`send_email`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->id_sendemail->setDbValue($conn->lastInsertId());
            $rs['id_sendemail'] = $this->id_sendemail->DbValue;
        }
        return $success;
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('id_sendemail', $rs)) {
                AddFilter($where, QuotedName('id_sendemail', $this->Dbid) . '=' . QuotedValue($rs['id_sendemail'], $this->id_sendemail->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->id_sendemail->DbValue = $row['id_sendemail'];
        ...
        $this->de_user->DbValue = $row['de_user'];
        $this->copy_user->DbValue = $row['copy_user'];
        $this->para_user->DbValue = $row['para_user'];
    }
    ...
    {
        return "`id_sendemail` = @id_sendemail@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_sendemail->CurrentValue : $this->id_sendemail->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_sendemail->CurrentValue = $keys[0];
            } else {
                $this->id_sendemail->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_sendemail', $row) ? $row['id_sendemail'] : null;
        } else {
            $val = $this->id_sendemail->OldValue !== null ? $this->id_sendemail->OldValue : $this->id_sendemail->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_sendemail@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("SendEmail2List");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "SendEmail2Edit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "SendEmail2Add") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "SendEmail2View";
            case Config("API_ADD_ACTION"):
                return "SendEmail2Add";
            case Config("API_EDIT_ACTION"):
                return "SendEmail2Edit";
            case Config("API_DELETE_ACTION"):
                return "SendEmail2Delete";
            case Config("API_LIST_ACTION"):
                return "SendEmail2List";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SendEmail2View", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("SendEmail2View", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "SendEmail2Add?" . $this->getUrlParm($parm);
        } else {
            $url = "SendEmail2Add";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("SendEmail2Edit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SendEmail2Add", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("SendEmail2Delete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "id_sendemail:" . JsonEncode($this->id_sendemail->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_sendemail->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->id_sendemail->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("id_sendemail") ?? Route("id_sendemail")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->id_sendemail->CurrentValue = $key;
            } else {
                $this->id_sendemail->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->copy_user->EditValue = $this->copy_user->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->id_sendemail);
                    ...
                    $doc->exportCaption($this->de_user);
                    $doc->exportCaption($this->copy_user);
                    $doc->exportCaption($this->para_user);
                } else {
                    $doc->exportCaption($this->id_sendemail);
                    $doc->exportCaption($this->sujeto);
                    $doc->exportCaption($this->tiempo);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->id_sendemail);
                        $doc->exportField($this->sujeto);
                        $doc->exportField($this->mensaje);
                        $doc->exportField($this->tiempo);
                        $doc->exportField($this->archivo);
                        $doc->exportField($this->status);
                        $doc->exportField($this->de_user);
                        $doc->exportField($this->copy_user);
                        $doc->exportField($this->para_user);
                    } else {
                        $doc->exportField($this->id_sendemail);
                        $doc->exportField($this->sujeto);
                        $doc->exportField($this->tiempo);
    ...
    public function getFileData($fldparm, $key, $resize, $width = 0, $height = 0, $plugins = [])
    {
        ...
        if (count($ar) == 1) {
            $this->id_sendemail->CurrentValue = $ar[0];
        } else {
