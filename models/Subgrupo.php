...
class Subgrupo extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->id_subgrupo->IsForeignKey = true; // Foreign key field
        ...
        $this->imagen_subgrupo->ImageResize = true;
        ...
        $this->nombre_subgrupo->Required = true; // Required field
        ...
        $this->descripcion_subgrupo = new DbField('subgrupo', 'subgrupo', 'x_descripcion_subgrupo', 'descripcion_subgrupo', '`descripcion_subgrupo`', '`descripcion_subgrupo`', 201, 65535, -1, false, '`descripcion_subgrupo`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "users") {
            ...
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id_subgrupo", $this->id_subgrupo->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "SubgrupoList";
        }
        return $detailUrl;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['id_subgrupo']) && $rsold['id_subgrupo'] != $rs['id_subgrupo'])) { // Update detail field 'subgrupo'
            $cascadeUpdate = true;
            $rscascade['subgrupo'] = $rs['id_subgrupo'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("users")->loadRs("`subgrupo` = " . QuotedValue($rsold['id_subgrupo'], DATATYPE_NUMBER, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        $dtlrows = Container("users")->loadRs("`subgrupo` = " . QuotedValue($rs['id_subgrupo'], DATATYPE_NUMBER, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
    ...
    public function getEditUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("SubgrupoEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("SubgrupoAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
