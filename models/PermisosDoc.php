...
class PermisosDoc extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // id_usuarios
        $this->id_usuarios = new DbField('permisos_doc', 'permisos_doc', 'x_id_usuarios', 'id_usuarios', '`id_usuarios`', '`id_usuarios`', 200, 60, -1, false, '`id_usuarios`', false, false, false, 'FORMATTED TEXT', 'CHECKBOX');
        $this->id_usuarios->Sortable = true; // Allow sort
        switch ($CurrentLanguage) {
            case "en":
                $this->id_usuarios->Lookup = new Lookup('id_usuarios', 'view_from', false, 'id', ["nombre_agrupacion","","",""], [], [], [], [], [], [], '', '');
                break;
            case "es":
                $this->id_usuarios->Lookup = new Lookup('id_usuarios', 'view_from', false, 'id', ["nombre_agrupacion","","",""], [], [], [], [], [], [], '', '');
                break;
    ...
    public function applyUserIDFilters($filter)
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            if ($this->getCurrentMasterTable() == "archivos_doc" || $this->getCurrentMasterTable() == "") {
                $filter = $this->addDetailUserIDFilter($filter, "archivos_doc"); // Add detail User ID filter
            }
        }
        return $filter;
    }
...
SORTHTML;
        ...
        if ($curVal != "") {
            ...
            if ($this->id_usuarios->ViewValue === null) { // Lookup from database
                ...
                $lookupFilter = function() {
                    return "`id` <>  '"."P-".CurrentUserID()."'";
                };
                $lookupFilter = $lookupFilter->bindTo($this);
                $sqlWrk = $this->id_usuarios->Lookup->getSql(false, $filterWrk, $lookupFilter, $this, true, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $this->id_usuarios->ViewValue = new OptionValues();
                    foreach ($rswrk as $row) {
                        $arwrk = $this->id_usuarios->Lookup->renderViewRow($row);
                        $this->id_usuarios->ViewValue->add($this->id_usuarios->displayValue($arwrk));
                    }
                } else {
    ...
    {
        $filterWrk = $filter;
        if ($currentMasterTable == "archivos_doc") {
            $filterWrk = Container("archivos_doc")->addUserIDFilter($filterWrk);
        }
        return $filterWrk;
    }
    ...
    public function addDetailUserIDFilter($filter, $currentMasterTable)
    {
        $filterWrk = $filter;
        if ($currentMasterTable == "archivos_doc") {
            $mastertable = Container("archivos_doc");
            if (!$mastertable->userIdAllow()) {
                $subqueryWrk = $mastertable->getUserIDSubquery($this->id_file, $mastertable->id_file);
                ...
                foreach ($tmp as &$valor) {
                    ...
                    if ($cod_tipo <> '') {
                        if ($tipo == 'G') { //grupos
                            ...
                            $sql = Execute("INSERT INTO permisos_docusers (id_file,tipo_permiso,id_users,date_created) VALUES('" . $rsnew['id_file'] . "','" . $rsnew['tipo_permiso'] . "','" . $cod_tipo . "','" . $rsnew['fecha_created'] . "');");
                        }
                        foreach ($usuarios as $row) {
                            $sql = Execute("INSERT INTO permisos_docusers (id_file,tipo_permiso,id_users,date_created) VALUES('" . $rsnew['id_file'] . "','" . $rsnew['tipo_permiso'] . "','" . $row['id_users'] . "','" . $rsnew['fecha_created'] . "');");
                        }
                    }
                }
            }
