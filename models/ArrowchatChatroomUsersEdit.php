 ...
 * Page class
 */
class ArrowchatChatroomUsersEdit extends ArrowchatChatroomUsers
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ArrowchatChatroomUsersEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("chatroom_id") ?? Key(1) ?? Route(3)) !== null) {
                $this->chatroom_id->setQueryStringValue($keyValue);
                $this->chatroom_id->setOldValue($this->chatroom_id->QueryStringValue);
            } elseif (Post("chatroom_id") !== null) {
                $this->chatroom_id->setFormValue(Post("chatroom_id"));
                $this->chatroom_id->setOldValue($this->chatroom_id->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_chatroom_id")) {
            $this->chatroom_id->setOldValue($CurrentForm->getValue("o_chatroom_id"));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // block_chats
            $this->block_chats->setDbValueDef($rsnew, $this->block_chats->CurrentValue, 0, $this->block_chats->ReadOnly);

            // silence_length
            $this->silence_length->setDbValueDef($rsnew, $this->silence_length->CurrentValue, null, $this->silence_length->ReadOnly);

            // silence_time
            $this->silence_time->setDbValueDef($rsnew, $this->silence_time->CurrentValue, null, $this->silence_time->ReadOnly);
