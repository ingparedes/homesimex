...
class Tareas extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // id_grupo
        $this->id_grupo = new DbField('tareas', 'tareas', 'x_id_grupo', 'id_grupo', '`id_grupo`', '`id_grupo`', 3, 11, -1, false, '`id_grupo`', false, false, false, 'FORMATTED TEXT', 'CHECKBOX');
        $this->id_grupo->Required = true; // Required field
        ...
        $this->titulo_tarea->Required = true; // Required field
        ...
        $this->fechainireal_tarea->Required = true; // Required field
        ...
        $this->fechafin_tarea->Required = true; // Required field
        ...
        $this->fechainisimulado_tarea->Required = true; // Required field
        ...
        $this->fechafinsimulado_tarea->Required = true; // Required field
        ...
        // valoracion
        $this->valoracion = new DbField('tareas', 'tareas', 'x_valoracion', 'valoracion', '`valoracion`', '`valoracion`', 3, 2, -1, false, '`valoracion`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->valoracion->Sortable = true; // Allow sort
        $this->valoracion->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->valoracion->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->valoracion->Param, "CustomMsg");
        $this->Fields['valoracion'] = &$this->valoracion;

        // color
        $this->color = new DbField('tareas', 'tareas', 'x_color', 'color', '`color`', '`color`', 200, 50, -1, false, '`color`', false, false, false, 'FORMATTED TEXT', 'SELECT');
    ...
    public function getSqlWhere() // Where
    {
        ...
        $this->DefaultFilter = (CurrentUserInfo("perfil") > 1) ? "`id_grupo` = '".CurrentUserInfo("grupo")."'" : "`id_escenario` = '".Container("escenario")->id_escenario->CurrentValue."'";
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->valoracion->DbValue = $row['valoracion'];
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->valoracion->EditValue = $this->valoracion->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->valoracion);
                } else {
                    $doc->exportCaption($this->id_tarea);
                    $doc->exportCaption($this->id_escenario);
                    $doc->exportCaption($this->id_grupo);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->valoracion);
                    } else {
                        $doc->exportField($this->id_tarea);
                        $doc->exportField($this->id_escenario);
                        $doc->exportField($this->id_grupo);
    ...
    public function recordsetSelecting(&$filter)
    {
      ...
      /*  $UserGroupotarea = CurrentUserInfo("grupo"); 
    */
       $myarray = $_GET['x_id_grupo'];
    //var_dump($myArray);
    foreach ($myarray as $valor) {
         $valor; };
    /*
     	if(empty($valor)){
     	// AddFilter($filter, "`id_grupo`  IN (".$valor.")");
         };
         else{
         AddFilter($filter, "`id_grupo`  = 60")
         };  */
        if(!empty($valor)){
         	AddFilter($filter, "`id_grupo`  IN (".$valor.")");
             };
