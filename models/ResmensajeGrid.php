 ...
 * Page class
 */
class ResmensajeGrid extends Resmensaje
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ResmensajeGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fresmensajegrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->id_resmensaje->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_id_inyect") && $CurrentForm->hasValue("o_id_inyect") && $this->id_inyect->CurrentValue != $this->id_inyect->OldValue) {
            return false;
        }
        if (!EmptyValue($this->resadjunto->Upload->Value)) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->id_resmensaje->clearErrorMessage();
        $this->id_users->clearErrorMessage();
        $this->id_inyect->clearErrorMessage();
        $this->resadjunto->clearErrorMessage();
    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->resadjunto->Upload->Index = $this->RowIndex;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_resmensaje->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->id_resmensaje->setFormValue($val);
        }
        ...
        if ($CurrentForm->hasValue("o_id_users")) {
            $this->id_users->setOldValue($CurrentForm->getValue("o_id_users"));
        }
        ...
        if ($CurrentForm->hasValue("o_id_inyect")) {
            $this->id_inyect->setOldValue($CurrentForm->getValue("o_id_inyect"));
        }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->id_resmensaje->CurrentValue = $this->id_resmensaje->FormValue;
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (is_numeric($this->RowIndex)) {
                RenderUploadField($this->resadjunto, $this->RowIndex);
            }
            ...
            // id_resmensaje
            $this->id_resmensaje->EditAttrs["class"] = "form-control";
            $this->id_resmensaje->EditCustomAttributes = "";
            $this->id_resmensaje->EditValue = $this->id_resmensaje->CurrentValue;
            $this->id_resmensaje->ViewCustomAttributes = "";
    ...
    protected function validateForm()
    {
        ...
        if ($this->id_resmensaje->Required) {
            if (!$this->id_resmensaje->IsDetailKey && EmptyValue($this->id_resmensaje->FormValue)) {
                $this->id_resmensaje->addErrorMessage(str_replace("%s", $this->id_resmensaje->caption(), $this->id_resmensaje->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // id_inyect
            if ($this->id_inyect->getSessionValue() != "") {
                $this->id_inyect->ReadOnly = true;
            }
            $this->id_inyect->setDbValueDef($rsnew, $this->id_inyect->CurrentValue, null, $this->id_inyect->ReadOnly);

            // resadjunto
            if ($this->resadjunto->Visible && !$this->resadjunto->ReadOnly && !$this->resadjunto->Upload->KeepFile) {
                $this->resadjunto->Upload->DbValue = $rsold['resadjunto']; // Get original value
            ...
            $keyValue = $rsnew['id_inyect'] ?? $rsold['id_inyect'];
            if (strval($keyValue) != "") {
                $masterFilter = str_replace("@id_inyect@", AdjustSql($keyValue), $masterFilter);
            } else {
            ...
            if ($this->resadjunto->Visible && !$this->resadjunto->Upload->KeepFile) {
                ...
                if (!EmptyValue($this->resadjunto->Upload->FileName)) {
                    ...
                    $this->resadjunto->setDbValueDef($rsnew, $this->resadjunto->Upload->FileName, null, $this->resadjunto->ReadOnly);
                }
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "mensajes") {
            $this->id_inyect->CurrentValue = $this->id_inyect->getSessionValue();
        }
