...
class Email2Add extends Email2
{
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->estado_msg->CurrentValue = null;
        $this->estado_msg->OldValue = $this->estado_msg->CurrentValue;
    ...
    protected function newRow()
    {
        ...
        $row['estado_msg'] = $this->estado_msg->CurrentValue;
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->archivo->Visible && !$this->archivo->Upload->KeepFile) {
            ...
            if (!EmptyValue($this->archivo->Upload->FileName)) {
                $newFiles = explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), strval($this->archivo->Upload->FileName));
        ...
        if ($insertRow) {
            ...
            if ($addRow) {
                if ($this->archivo->Visible && !$this->archivo->Upload->KeepFile) {
                    $oldFiles = EmptyValue($this->archivo->Upload->DbValue) ? [] : explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $this->archivo->htmlDecode(strval($this->archivo->Upload->DbValue)));
                    if (!EmptyValue($this->archivo->Upload->FileName)) {
                        $newFiles = explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $this->archivo->Upload->FileName);
                        $newFiles2 = explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $this->archivo->htmlDecode($rsnew['archivo']));
                        $newFileCount = count($newFiles);
                        for ($i = 0; $i < $newFileCount; $i++) {
                            if ($newFiles[$i] != "") {
    ...
    {
        //Log("Page Load");
             $this->sender_userid->DisplayValueSeparator = " "; // Use hyphen as separator
    ...
    $fecha =  ExecuteScalar("SELECT tiempo FROM email WHERE id_email =".$IdEmail);
    $nombre = ExecuteScalar("SELECT users.nombres FROM email INNER JOIN users ON email.reciever_userid = users.id_users WHERE email.id_email =".$IdEmail);
    $apellidos = ExecuteScalar("SELECT users.apellidos FROM email INNER JOIN users ON email.reciever_userid = users.id_users WHERE email.id_email =".$IdEmail);
    SetClientVar("nom",$nombre );
    SetClientVar("titles",$title );
    SetClientVar("bodys",$body );
    SetClientVar("remitente",$remite );
    ...
    if (!empty($reenviar))
    {
    $title = ExecuteScalar("SELECT  sujeto FROM email WHERE id_email =".$reenviar);
    $body = ExecuteScalar("SELECT  mensaje FROM email WHERE id_email =".$reenviar);
    $remite = ExecuteScalar("SELECT  reciever_userid FROM email WHERE id_email =".$reenviar);
    $adj =  ExecuteScalar("SELECT archivo FROM email WHERE id_email =".$reenviar);
    $fecha =  ExecuteScalar("SELECT tiempo FROM email WHERE id_email =".$reenviar);
    $nombre = ExecuteScalar("SELECT users.nombres FROM email INNER JOIN users ON email.reciever_userid = users.id_users WHERE email.id_email =".$reenviar);
    $apellidos = ExecuteScalar("SELECT users.apellidos FROM email INNER JOIN users ON email.reciever_userid = users.id_users WHERE email.id_email =".$reenviar);
    SetClientVar("nom",$nombre );
    SetClientVar("titles",$title );
    SetClientVar("bodys",$body );
    SetClientVar("remitente",$remite );
