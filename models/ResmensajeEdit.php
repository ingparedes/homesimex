 ...
 * Page class
 */
class ResmensajeEdit extends Resmensaje
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ResmensajeEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_resmensaje") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_resmensaje->setQueryStringValue($keyValue);
                $this->id_resmensaje->setOldValue($this->id_resmensaje->QueryStringValue);
            } elseif (Post("id_resmensaje") !== null) {
                $this->id_resmensaje->setFormValue(Post("id_resmensaje"));
                $this->id_resmensaje->setOldValue($this->id_resmensaje->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_resmensaje->IsDetailKey) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // resmensaje
            $this->resmensaje->setDbValueDef($rsnew, $this->resmensaje->CurrentValue, null, $this->resmensaje->ReadOnly);
