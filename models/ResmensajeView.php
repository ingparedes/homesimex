 ...
 * Page class
 */
class ResmensajeView extends Resmensaje
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ResmensajeView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("id_resmensaje") ?? Route("id_resmensaje")) !== null) {
            $this->RecKey["id_resmensaje"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_resmensaje") ?? Route("id_resmensaje")) !== null) {
                ...
                $this->RecKey["id_resmensaje"] = $this->id_resmensaje->QueryStringValue;
            } elseif (Post("id_resmensaje") !== null) {
                $this->id_resmensaje->setFormValue(Post("id_resmensaje"));
                $this->RecKey["id_resmensaje"] = $this->id_resmensaje->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_resmensaje->setQueryStringValue($keyValue);
                $this->RecKey["id_resmensaje"] = $this->id_resmensaje->QueryStringValue;
            } else {
                $returnUrl = "ResmensajeList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "ResmensajeList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "ResmensajeList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
