...
// Page object
$ArrowchatThemesAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var farrowchat_themesadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    farrowchat_themesadd = currentForm = new ew.Form("farrowchat_themesadd", "add");
    ...
    farrowchat_themesadd.addFields([
    ...
    // Validate form
    farrowchat_themesadd.validate = function () {
    ...
    // Form_CustomValidate
    farrowchat_themesadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farrowchat_themesadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    farrowchat_themesadd.lists.active = <?= $Page->active->toClientList($Page) ?>;
    farrowchat_themesadd.lists._default = <?= $Page->_default->toClientList($Page) ?>;
    loadjs.done("farrowchat_themesadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="farrowchat_themesadd" id="farrowchat_themesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->active->isInvalidClass() ?>" data-table="arrowchat_themes" data-field="x_active" name="x_active[]" id="x_active_285313" value="1"<?= ConvertToBool($Page->active->CurrentValue) ? " checked" : "" ?><?= $Page->active->editAttributes() ?> aria-describedby="x_active_help">
    <label class="custom-control-label" for="x_active_285313"></label>
</div>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->_default->isInvalidClass() ?>" data-table="arrowchat_themes" data-field="x__default" name="x__default[]" id="x__default_174803" value="1"<?= ConvertToBool($Page->_default->CurrentValue) ? " checked" : "" ?><?= $Page->_default->editAttributes() ?> aria-describedby="x__default_help">
    <label class="custom-control-label" for="x__default_174803"></label>
</div>
