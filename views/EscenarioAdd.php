...
loadjs.ready("head", function () {
    ...
    fescenarioadd.addFields([
        ...
        ["evento_asociado", [fields.evento_asociado.visible && fields.evento_asociado.required ? ew.Validators.required(fields.evento_asociado.caption) : null], fields.evento_asociado.isInvalid],
    ...
    fescenarioadd.lists.evento_asociado = <?= $Page->evento_asociado->toClientList($Page) ?>;
    fescenarioadd.lists.pais_escenario = <?= $Page->pais_escenario->toClientList($Page) ?>;
    fescenarioadd.lists.estado = <?= $Page->estado->toClientList($Page) ?>;
...
loadjs.ready("head", function () {
    // Client script
    $(document).on("create.editor",(function(e,t){t.settings.height="400px",t.settings.allowedContent=!0,t.settings.extraAllowedContent="iframe[*]",t.settings.extraPlugins="templates",t.settings.toolbarGroups=[{name:"document",groups:["mode","document","doctools"]},{name:"clipboard",groups:["clipboard","undo"]},{name:"editing",groups:["find","selection","spellchecker","editing"]},{name:"forms",groups:["forms"]},"/",{name:"basicstyles",groups:["basicstyles","cleanup"]},{name:"paragraph",groups:["list","indent","blocks","align","bidi","paragraph"]},{name:"links",groups:["links"]},{name:"insert",groups:["insert"]},"/",{name:"styles",groups:["styles"]},{name:"colors",groups:["colors"]},{name:"tools",groups:["tools"]},{name:"others",groups:["others"]},{name:"about",groups:["about"]}],t.settings.removeButtons="Source,Save,NewPage,ExportPdf,Cut,Copy,Paste,PasteText,PasteFromWord,Find,Replace,SelectAll,Form,Checkbox,Radio,TextField,Textarea,Select,Button,ImageButton,HiddenField,Blockquote,CreateDiv,BidiLtr,BidiRtl,Language,Link,Unlink,Anchor,Flash,Table,HorizontalRule,Smiley,SpecialChar,PageBreak,About"}));
});
</script>
...
$Page->showMessage();
?>
<form name="fescenarioadd" id="fescenarioadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->evento_asociado->Visible) { // evento_asociado ?>
    <div id="r_evento_asociado" class="form-group row">
        <label id="elh_escenario_evento_asociado" for="x_evento_asociado" class="<?= $Page->LeftColumnClass ?>"><?= $Page->evento_asociado->caption() ?><?= $Page->evento_asociado->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->evento_asociado->cellAttributes() ?>>
<span id="el_escenario_evento_asociado">
    <select
        id="x_evento_asociado[]"
        name="x_evento_asociado[]"
        class="form-control ew-select<?= $Page->evento_asociado->isInvalidClass() ?>"
        data-select2-id="escenario_x_evento_asociado[]"
        data-table="escenario"
        data-field="x_evento_asociado"
        multiple
        size="1"
        data-value-separator="<?= $Page->evento_asociado->displayValueSeparatorAttribute() ?>"
        data-placeholder="<?= HtmlEncode($Page->evento_asociado->getPlaceHolder()) ?>"
        <?= $Page->evento_asociado->editAttributes() ?>>
        <?= $Page->evento_asociado->selectOptionListHtml("x_evento_asociado[]") ?>
    </select>
    <?= $Page->evento_asociado->getCustomMessage() ?>
    <div class="invalid-feedback"><?= $Page->evento_asociado->getErrorMessage() ?></div>
<?= $Page->evento_asociado->Lookup->getParamTag($Page, "p_x_evento_asociado") ?>
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='escenario_x_evento_asociado[]']"),
        options = { name: "x_evento_asociado[]", selectId: "escenario_x_evento_asociado[]", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.multiple = true;
    options.closeOnSelect = false;
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.escenario.fields.evento_asociado.selectOptions);
...
<input type="hidden" name="fs_x_image_escenario" id= "fs_x_image_escenario" value="200">
