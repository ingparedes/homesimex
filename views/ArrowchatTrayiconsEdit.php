...
// Page object
$ArrowchatTrayiconsEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var farrowchat_trayiconsedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    farrowchat_trayiconsedit = currentForm = new ew.Form("farrowchat_trayiconsedit", "edit");
    ...
    farrowchat_trayiconsedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = farrowchat_trayiconsedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    farrowchat_trayiconsedit.validate = function () {
    ...
    // Form_CustomValidate
    farrowchat_trayiconsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farrowchat_trayiconsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    farrowchat_trayiconsedit.lists.active = <?= $Page->active->toClientList($Page) ?>;
    loadjs.done("farrowchat_trayiconsedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="farrowchat_trayiconsedit" id="farrowchat_trayiconsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_arrowchat_trayicons_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_arrowchat_trayicons_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="arrowchat_trayicons" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->active->isInvalidClass() ?>" data-table="arrowchat_trayicons" data-field="x_active" name="x_active[]" id="x_active_386019" value="1"<?= ConvertToBool($Page->active->CurrentValue) ? " checked" : "" ?><?= $Page->active->editAttributes() ?> aria-describedby="x_active_help">
    <label class="custom-control-label" for="x_active_386019"></label>
</div>
