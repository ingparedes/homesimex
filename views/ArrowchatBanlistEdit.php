...
// Page object
$ArrowchatBanlistEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var farrowchat_banlistedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    farrowchat_banlistedit = currentForm = new ew.Form("farrowchat_banlistedit", "edit");
    ...
    farrowchat_banlistedit.addFields([
        ["ban_id", [fields.ban_id.visible && fields.ban_id.required ? ew.Validators.required(fields.ban_id.caption) : null], fields.ban_id.isInvalid],
    ...
    // Validate form
    farrowchat_banlistedit.validate = function () {
    ...
    // Form_CustomValidate
    farrowchat_banlistedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farrowchat_banlistedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("farrowchat_banlistedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="farrowchat_banlistedit" id="farrowchat_banlistedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->ban_id->Visible) { // ban_id ?>
    <div id="r_ban_id" class="form-group row">
        <label id="elh_arrowchat_banlist_ban_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ban_id->caption() ?><?= $Page->ban_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->ban_id->cellAttributes() ?>>
<span id="el_arrowchat_banlist_ban_id">
<span<?= $Page->ban_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->ban_id->getDisplayValue($Page->ban_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="arrowchat_banlist" data-field="x_ban_id" data-hidden="1" name="x_ban_id" id="x_ban_id" value="<?= HtmlEncode($Page->ban_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
