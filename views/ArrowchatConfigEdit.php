...
// Page object
$ArrowchatConfigEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var farrowchat_configedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    farrowchat_configedit = currentForm = new ew.Form("farrowchat_configedit", "edit");
    ...
    farrowchat_configedit.addFields([
    ...
    // Validate form
    farrowchat_configedit.validate = function () {
    ...
    // Form_CustomValidate
    farrowchat_configedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farrowchat_configedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    farrowchat_configedit.lists.is_dynamic = <?= $Page->is_dynamic->toClientList($Page) ?>;
    loadjs.done("farrowchat_configedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="farrowchat_configedit" id="farrowchat_configedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="arrowchat_config" data-field="x_config_name" data-hidden="1" name="o_config_name" id="o_config_name" value="<?= HtmlEncode($Page->config_name->OldValue ?? $Page->config_name->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->is_dynamic->isInvalidClass() ?>" data-table="arrowchat_config" data-field="x_is_dynamic" name="x_is_dynamic[]" id="x_is_dynamic_538586" value="1"<?= ConvertToBool($Page->is_dynamic->CurrentValue) ? " checked" : "" ?><?= $Page->is_dynamic->editAttributes() ?> aria-describedby="x_is_dynamic_help">
    <label class="custom-control-label" for="x_is_dynamic_538586"></label>
</div>
