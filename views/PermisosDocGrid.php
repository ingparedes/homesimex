...
<div class="input-group ew-lookup-list">
    <div class="form-control ew-lookup-text" tabindex="-1" id="lu_x<?= $Grid->RowIndex ?>_id_usuarios"><?= EmptyValue(strval($Grid->id_usuarios->ViewValue)) ? $Language->phrase("PleaseSelect") : $Grid->id_usuarios->ViewValue ?></div>
    <div class="input-group-append">
        <button type="button" title="<?= HtmlEncode(str_replace("%s", RemoveHtml($Grid->id_usuarios->caption()), $Language->phrase("LookupLink", true))) ?>" class="ew-lookup-btn btn btn-default"<?= ($Grid->id_usuarios->ReadOnly || $Grid->id_usuarios->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x<?= $Grid->RowIndex ?>_id_usuarios[]',m:1,n:10});"><i class="fas fa-search ew-icon"></i></button>
    </div>
</div>
...
<input type="hidden" is="selection-list" data-table="permisos_doc" data-field="x_id_usuarios" data-type="text" data-multiple="1" data-lookup="1" data-value-separator="<?= $Grid->id_usuarios->displayValueSeparatorAttribute() ?>" name="x<?= $Grid->RowIndex ?>_id_usuarios[]" id="x<?= $Grid->RowIndex ?>_id_usuarios[]" value="<?= $Grid->id_usuarios->CurrentValue ?>"<?= $Grid->id_usuarios->editAttributes() ?>>
</span>
