...
// Page object
$ArrowchatChatroomUsersEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var farrowchat_chatroom_usersedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    farrowchat_chatroom_usersedit = currentForm = new ew.Form("farrowchat_chatroom_usersedit", "edit");
    ...
    farrowchat_chatroom_usersedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = farrowchat_chatroom_usersedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    farrowchat_chatroom_usersedit.validate = function () {
    ...
    // Form_CustomValidate
    farrowchat_chatroom_usersedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farrowchat_chatroom_usersedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    farrowchat_chatroom_usersedit.lists.is_admin = <?= $Page->is_admin->toClientList($Page) ?>;
    farrowchat_chatroom_usersedit.lists.is_mod = <?= $Page->is_mod->toClientList($Page) ?>;
    loadjs.done("farrowchat_chatroom_usersedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="farrowchat_chatroom_usersedit" id="farrowchat_chatroom_usersedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="arrowchat_chatroom_users" data-field="x_user_id" data-hidden="1" name="o_user_id" id="o_user_id" value="<?= HtmlEncode($Page->user_id->OldValue ?? $Page->user_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="arrowchat_chatroom_users" data-field="x_chatroom_id" data-hidden="1" name="o_chatroom_id" id="o_chatroom_id" value="<?= HtmlEncode($Page->chatroom_id->OldValue ?? $Page->chatroom_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->is_admin->isInvalidClass() ?>" data-table="arrowchat_chatroom_users" data-field="x_is_admin" name="x_is_admin[]" id="x_is_admin_966575" value="1"<?= ConvertToBool($Page->is_admin->CurrentValue) ? " checked" : "" ?><?= $Page->is_admin->editAttributes() ?> aria-describedby="x_is_admin_help">
    <label class="custom-control-label" for="x_is_admin_966575"></label>
</div>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->is_mod->isInvalidClass() ?>" data-table="arrowchat_chatroom_users" data-field="x_is_mod" name="x_is_mod[]" id="x_is_mod_995922" value="1"<?= ConvertToBool($Page->is_mod->CurrentValue) ? " checked" : "" ?><?= $Page->is_mod->editAttributes() ?> aria-describedby="x_is_mod_help">
    <label class="custom-control-label" for="x_is_mod_995922"></label>
</div>
