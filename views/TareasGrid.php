...
<template id="tp_x<?= $Grid->RowIndex ?>_id_grupo">
    <div class="custom-control custom-checkbox">
        <input type="checkbox" class="custom-control-input" data-table="tareas" data-field="x_id_grupo" name="x<?= $Grid->RowIndex ?>_id_grupo" id="x<?= $Grid->RowIndex ?>_id_grupo"<?= $Grid->id_grupo->editAttributes() ?>>
        <label class="custom-control-label"></label>
    </div>
</template>
<div id="dsl_x<?= $Grid->RowIndex ?>_id_grupo" class="ew-item-list"></div>
<input type="hidden"
    is="selection-list"
    id="x<?= $Grid->RowIndex ?>_id_grupo[]"
    name="x<?= $Grid->RowIndex ?>_id_grupo[]"
    value="<?= HtmlEncode($Grid->id_grupo->CurrentValue) ?>"
    data-type="select-multiple"
    data-template="tp_x<?= $Grid->RowIndex ?>_id_grupo"
    data-target="dsl_x<?= $Grid->RowIndex ?>_id_grupo"
    data-repeatcolumn="6"
    class="form-control<?= $Grid->id_grupo->isInvalidClass() ?>"
    data-table="tareas"
...
<input type="<?= $Grid->titulo_tarea->getInputTextType() ?>" data-table="tareas" data-field="x_titulo_tarea" name="x<?= $Grid->RowIndex ?>_titulo_tarea" id="x<?= $Grid->RowIndex ?>_titulo_tarea" size="100" maxlength="100" placeholder="<?= HtmlEncode($Grid->titulo_tarea->getPlaceHolder()) ?>" value="<?= $Grid->titulo_tarea->EditValue ?>"<?= $Grid->titulo_tarea->editAttributes() ?>>
...
loadjs.ready("load", function () {
    // Startup script
    $("#r_icon_escenario").hide(),$("#r_fechacreacion_escenario").hide(),$("#r_incidente").hide(),$("#r_entrar").hide(),$("#ftareaslistsrch").before('<div class="callout callout-primary"><em><?php echo $sinecesita; ?></em></div>');
});
</script>
<?php } ?>
