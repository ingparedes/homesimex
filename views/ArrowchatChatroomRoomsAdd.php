...
// Page object
$ArrowchatChatroomRoomsAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var farrowchat_chatroom_roomsadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    farrowchat_chatroom_roomsadd = currentForm = new ew.Form("farrowchat_chatroom_roomsadd", "add");
    ...
    farrowchat_chatroom_roomsadd.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = farrowchat_chatroom_roomsadd,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    farrowchat_chatroom_roomsadd.validate = function () {
    ...
    // Form_CustomValidate
    farrowchat_chatroom_roomsadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farrowchat_chatroom_roomsadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    farrowchat_chatroom_roomsadd.lists.type = <?= $Page->type->toClientList($Page) ?>;
    farrowchat_chatroom_roomsadd.lists.is_featured = <?= $Page->is_featured->toClientList($Page) ?>;
    loadjs.done("farrowchat_chatroom_roomsadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="farrowchat_chatroom_roomsadd" id="farrowchat_chatroom_roomsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->type->isInvalidClass() ?>" data-table="arrowchat_chatroom_rooms" data-field="x_type" name="x_type[]" id="x_type_219725" value="1"<?= ConvertToBool($Page->type->CurrentValue) ? " checked" : "" ?><?= $Page->type->editAttributes() ?> aria-describedby="x_type_help">
    <label class="custom-control-label" for="x_type_219725"></label>
</div>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->is_featured->isInvalidClass() ?>" data-table="arrowchat_chatroom_rooms" data-field="x_is_featured" name="x_is_featured[]" id="x_is_featured_116819" value="1"<?= ConvertToBool($Page->is_featured->CurrentValue) ? " checked" : "" ?><?= $Page->is_featured->editAttributes() ?> aria-describedby="x_is_featured_help">
    <label class="custom-control-label" for="x_is_featured_116819"></label>
</div>
