...
// Page object
$ArrowchatApplicationsAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var farrowchat_applicationsadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    farrowchat_applicationsadd = currentForm = new ew.Form("farrowchat_applicationsadd", "add");
    ...
    farrowchat_applicationsadd.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = farrowchat_applicationsadd,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    farrowchat_applicationsadd.validate = function () {
    ...
    // Form_CustomValidate
    farrowchat_applicationsadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farrowchat_applicationsadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    farrowchat_applicationsadd.lists.dont_reload = <?= $Page->dont_reload->toClientList($Page) ?>;
    farrowchat_applicationsadd.lists.default_bookmark = <?= $Page->default_bookmark->toClientList($Page) ?>;
    farrowchat_applicationsadd.lists.show_to_guests = <?= $Page->show_to_guests->toClientList($Page) ?>;
    farrowchat_applicationsadd.lists.active = <?= $Page->active->toClientList($Page) ?>;
    loadjs.done("farrowchat_applicationsadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="farrowchat_applicationsadd" id="farrowchat_applicationsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->show_to_guests->isInvalidClass() ?>" data-table="arrowchat_applications" data-field="x_show_to_guests" name="x_show_to_guests[]" id="x_show_to_guests_776768" value="1"<?= ConvertToBool($Page->show_to_guests->CurrentValue) ? " checked" : "" ?><?= $Page->show_to_guests->editAttributes() ?> aria-describedby="x_show_to_guests_help">
    <label class="custom-control-label" for="x_show_to_guests_776768"></label>
</div>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->active->isInvalidClass() ?>" data-table="arrowchat_applications" data-field="x_active" name="x_active[]" id="x_active_436024" value="1"<?= ConvertToBool($Page->active->CurrentValue) ? " checked" : "" ?><?= $Page->active->editAttributes() ?> aria-describedby="x_active_help">
    <label class="custom-control-label" for="x_active_436024"></label>
</div>
