...
loadjs.ready("head", function () {
    ...
    fmensajes_usuariosedit.lists.leido = <?= $Page->leido->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fmensajes_usuariosedit" id="fmensajes_usuariosedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->leido->Visible) { // leido ?>
    <div id="r_leido" class="form-group row">
        <label id="elh_mensajes_usuarios_leido" class="<?= $Page->LeftColumnClass ?>"><?= $Page->leido->caption() ?><?= $Page->leido->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->leido->cellAttributes() ?>>
<span id="el_mensajes_usuarios_leido">
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->leido->isInvalidClass() ?>" data-table="mensajes_usuarios" data-field="x_leido" name="x_leido[]" id="x_leido_110808" value="1"<?= ConvertToBool($Page->leido->CurrentValue) ? " checked" : "" ?><?= $Page->leido->editAttributes() ?> aria-describedby="x_leido_help">
    <label class="custom-control-label" for="x_leido_110808"></label>
</div>
