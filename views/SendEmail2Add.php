...
loadjs.ready("head", function () {
    ...
    fsend_email2add = currentForm = new ew.Form("fsend_email2add", "add");
    ...
    // Validate form
    fsend_email2add.validate = function () {
    ...
    // Form_CustomValidate
    fsend_email2add.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsend_email2add.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fsend_email2add.lists.status = <?= $Page->status->toClientList($Page) ?>;
    fsend_email2add.lists.para_user = <?= $Page->para_user->toClientList($Page) ?>;
    loadjs.done("fsend_email2add");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fsend_email2add" id="fsend_email2add" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->status->isInvalidClass() ?>" data-table="send_email2" data-field="x_status" name="x_status[]" id="x_status_803294" value="1"<?= ConvertToBool($Page->status->CurrentValue) ? " checked" : "" ?><?= $Page->status->editAttributes() ?> aria-describedby="x_status_help">
    <label class="custom-control-label" for="x_status_803294"></label>
</div>
