...
loadjs.ready("head", function () {
    ...
    fresmensajeedit = currentForm = new ew.Form("fresmensajeedit", "edit");
    ...
    // Validate form
    fresmensajeedit.validate = function () {
    ...
    // Form_CustomValidate
    fresmensajeedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fresmensajeedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fresmensajeedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fresmensajeedit" id="fresmensajeedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_resmensaje->Visible) { // id_resmensaje ?>
    <div id="r_id_resmensaje" class="form-group row">
        <label id="elh_resmensaje_id_resmensaje" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_resmensaje->caption() ?><?= $Page->id_resmensaje->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_resmensaje->cellAttributes() ?>>
<span id="el_resmensaje_id_resmensaje">
<span<?= $Page->id_resmensaje->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_resmensaje->getDisplayValue($Page->id_resmensaje->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="resmensaje" data-field="x_id_resmensaje" data-hidden="1" name="x_id_resmensaje" id="x_id_resmensaje" value="<?= HtmlEncode($Page->id_resmensaje->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" name="fa_x_resadjunto" id= "fa_x_resadjunto" value="<?= (Post("fa_x_resadjunto") == "0") ? "0" : "1" ?>">
