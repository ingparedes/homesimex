...
// Page object
$ArrowchatChatroomMessagesEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var farrowchat_chatroom_messagesedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    farrowchat_chatroom_messagesedit = currentForm = new ew.Form("farrowchat_chatroom_messagesedit", "edit");
    ...
    farrowchat_chatroom_messagesedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = farrowchat_chatroom_messagesedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    farrowchat_chatroom_messagesedit.validate = function () {
    ...
    // Form_CustomValidate
    farrowchat_chatroom_messagesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farrowchat_chatroom_messagesedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    farrowchat_chatroom_messagesedit.lists.global_message = <?= $Page->global_message->toClientList($Page) ?>;
    farrowchat_chatroom_messagesedit.lists.is_mod = <?= $Page->is_mod->toClientList($Page) ?>;
    farrowchat_chatroom_messagesedit.lists.is_admin = <?= $Page->is_admin->toClientList($Page) ?>;
    farrowchat_chatroom_messagesedit.lists._action = <?= $Page->_action->toClientList($Page) ?>;
    loadjs.done("farrowchat_chatroom_messagesedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="farrowchat_chatroom_messagesedit" id="farrowchat_chatroom_messagesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_arrowchat_chatroom_messages_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_arrowchat_chatroom_messages_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="arrowchat_chatroom_messages" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->is_mod->isInvalidClass() ?>" data-table="arrowchat_chatroom_messages" data-field="x_is_mod" name="x_is_mod[]" id="x_is_mod_850985" value="1"<?= ConvertToBool($Page->is_mod->CurrentValue) ? " checked" : "" ?><?= $Page->is_mod->editAttributes() ?> aria-describedby="x_is_mod_help">
    <label class="custom-control-label" for="x_is_mod_850985"></label>
</div>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->is_admin->isInvalidClass() ?>" data-table="arrowchat_chatroom_messages" data-field="x_is_admin" name="x_is_admin[]" id="x_is_admin_310651" value="1"<?= ConvertToBool($Page->is_admin->CurrentValue) ? " checked" : "" ?><?= $Page->is_admin->editAttributes() ?> aria-describedby="x_is_admin_help">
    <label class="custom-control-label" for="x_is_admin_310651"></label>
</div>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->_action->isInvalidClass() ?>" data-table="arrowchat_chatroom_messages" data-field="x__action" name="x__action[]" id="x__action_827611" value="1"<?= ConvertToBool($Page->_action->CurrentValue) ? " checked" : "" ?><?= $Page->_action->editAttributes() ?> aria-describedby="x__action_help">
    <label class="custom-control-label" for="x__action_827611"></label>
</div>
