...
loadjs.ready("head", function () {
    ...
    farrowchatedit = currentForm = new ew.Form("farrowchatedit", "edit");
    ...
    // Validate form
    farrowchatedit.validate = function () {
    ...
    // Form_CustomValidate
    farrowchatedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farrowchatedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    farrowchatedit.lists.user_read = <?= $Page->user_read->toClientList($Page) ?>;
    loadjs.done("farrowchatedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="farrowchatedit" id="farrowchatedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_arrowchat_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_arrowchat_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="arrowchat" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->user_read->isInvalidClass() ?>" data-table="arrowchat" data-field="x_user_read" name="x_user_read[]" id="x_user_read_524162" value="1"<?= ConvertToBool($Page->user_read->CurrentValue) ? " checked" : "" ?><?= $Page->user_read->editAttributes() ?> aria-describedby="x_user_read_help">
    <label class="custom-control-label" for="x_user_read_524162"></label>
</div>
