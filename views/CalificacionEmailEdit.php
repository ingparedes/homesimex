...
// Page object
$CalificacionEmailEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fcalificacion_emailedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fcalificacion_emailedit = currentForm = new ew.Form("fcalificacion_emailedit", "edit");
    ...
    fcalificacion_emailedit.addFields([
        ["id_calificacion_email", [fields.id_calificacion_email.visible && fields.id_calificacion_email.required ? ew.Validators.required(fields.id_calificacion_email.caption) : null], fields.id_calificacion_email.isInvalid],
    ...
    // Validate form
    fcalificacion_emailedit.validate = function () {
    ...
    // Form_CustomValidate
    fcalificacion_emailedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcalificacion_emailedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fcalificacion_emailedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fcalificacion_emailedit" id="fcalificacion_emailedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_calificacion_email->Visible) { // id_calificacion_email ?>
    <div id="r_id_calificacion_email" class="form-group row">
        <label id="elh_calificacion_email_id_calificacion_email" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_calificacion_email->caption() ?><?= $Page->id_calificacion_email->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_calificacion_email->cellAttributes() ?>>
<span id="el_calificacion_email_id_calificacion_email">
<span<?= $Page->id_calificacion_email->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_calificacion_email->getDisplayValue($Page->id_calificacion_email->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="calificacion_email" data-field="x_id_calificacion_email" data-hidden="1" name="x_id_calificacion_email" id="x_id_calificacion_email" value="<?= HtmlEncode($Page->id_calificacion_email->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fcalificacion_emailedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fcalificacion_emailedit", "x_create_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
