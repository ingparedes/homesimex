...
loadjs.ready("head", function () {
    ...
    fnotiemailedit = currentForm = new ew.Form("fnotiemailedit", "edit");
    ...
    fnotiemailedit.addFields([
        ["id_notiemail", [fields.id_notiemail.visible && fields.id_notiemail.required ? ew.Validators.required(fields.id_notiemail.caption) : null], fields.id_notiemail.isInvalid],
    ...
    // Validate form
    fnotiemailedit.validate = function () {
    ...
    // Form_CustomValidate
    fnotiemailedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fnotiemailedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fnotiemailedit.lists.leido = <?= $Page->leido->toClientList($Page) ?>;
    loadjs.done("fnotiemailedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fnotiemailedit" id="fnotiemailedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_notiemail->Visible) { // id_notiemail ?>
    <div id="r_id_notiemail" class="form-group row">
        <label id="elh_notiemail_id_notiemail" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_notiemail->caption() ?><?= $Page->id_notiemail->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_notiemail->cellAttributes() ?>>
<span id="el_notiemail_id_notiemail">
<span<?= $Page->id_notiemail->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_notiemail->getDisplayValue($Page->id_notiemail->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="notiemail" data-field="x_id_notiemail" data-hidden="1" name="x_id_notiemail" id="x_id_notiemail" value="<?= HtmlEncode($Page->id_notiemail->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->leido->isInvalidClass() ?>" data-table="notiemail" data-field="x_leido" name="x_leido[]" id="x_leido_512359" value="1"<?= ConvertToBool($Page->leido->CurrentValue) ? " checked" : "" ?><?= $Page->leido->editAttributes() ?> aria-describedby="x_leido_help">
    <label class="custom-control-label" for="x_leido_512359"></label>
</div>
